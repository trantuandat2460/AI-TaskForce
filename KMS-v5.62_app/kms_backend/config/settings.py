...
# ---- Lớp PM (v5.62) — quản lý dự án đặt TRÊN nền v5.6 ----------------------
APP_VERSION   = "v5.62"
PROJECT_ROLES = ["PM", "LEADER", "ENGINEER"]      # vai trò TRONG dự án (project-scoped)
TASK_STATUSES = ["todo", "doing", "review", "done", "blocked"]
TASK_PRIORITIES = ["P0", "P1", "P2", "P3"]
# Connector service-account: scope hiệu lực = min(clearance người gọi, SA) — chống
# confused-deputy (kỹ sư quyền thấp KHÔNG mượn được quyền SA để kéo dữ liệu cao hơn).
CONNECTOR_SA_CLEARANCE = "C2"
# Egress-by-tier: chỉ data_class ≤ mức này mới được rời enclave qua uplink đã duyệt
# (bản stub chạy offline; hằng số này để PEP connector quyết định cho production).
EGRESS_MAX_CLASS = "C2"
